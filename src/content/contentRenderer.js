 ...
 * Adapted for CodeMonkey course with code-block type.
 */
...
function renderBlock(block) {
  switch (block.type) {
    ...
    case 'code-block': return renderCodeBlock(block);
...
function renderCodeBlock(block) {
  const el = document.createElement('div');
  el.className = 'code-block-wrapper';
  el.innerHTML = `
    <div class="code-block-header">${block.label || 'CoffeeScript'}</div>
    <pre class="code-block"><code>${block.code}</code></pre>
    ${block.annotation ? `<p class="code-annotation">${block.annotation}</p>` : ''}
  `;
  return el;
}
